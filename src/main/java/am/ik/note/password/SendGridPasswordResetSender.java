...
public class SendGridPasswordResetSender implements PasswordResetSender {
	...
	public SendGridPasswordResetSender(SendGridSender sendGridSender, ReaderMapper readerMapper) {
	...
	public void sendLink(PasswordReset passwordReset) {
		this.readerMapper.findById(passwordReset.readerId())
				.ifPresent(reader -> {
					final String to = reader.email();
					final String subject = "【はじめるSpring Boot 3】パスワードリセットリンク通知";
					final String link = String.format("https://ik.am/note/password_reset/%s", passwordReset.resetId());
