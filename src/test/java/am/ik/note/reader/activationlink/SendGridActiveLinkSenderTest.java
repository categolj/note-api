...
class SendGridActiveLinkSenderTest {
	...
	void sendActivationLink() {
		...
		final SendGridActiveLinkSender sendGridActiveLinkSender = new SendGridActiveLinkSender(sendGridSender);
		...
		final ActivationLink activationLink = new ActivationLink(activationLinkId, readerId, created);
		...
		assertThat(subject.getValue()).isEqualTo("【はじめるSpring Boot 3】アカウントアクティベーションリンク通知");
