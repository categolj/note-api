...
class SendGridPasswordResetSenderTest {
	...
	void sendLink() {
		...
		final SendGridPasswordResetSender passwordResetSender = new SendGridPasswordResetSender(sendGridSender, readerMapper);
		...
		final PasswordReset passwordReset = new PasswordReset(passwordResetId, readerId, created);
		given(readerMapper.findById(readerId)).willReturn(Optional.of(new Reader(readerId, "demo@example.com", null, null)));
		...
		verify(sendGridSender).sendMail(to.capture(), subject.capture(), content.capture());
