...
class NoteControllerTest {
	...
	void getNotes_200() throws Exception {
		...
		given(this.noteService.findAll(readerId)).willReturn(List.of(new NoteSummaryBuilder()
						.withNoteId(noteId1)
						.withEntryId(101L)
						.withTitle("title1")
	...
	void getNoteByEntryId_200() throws Exception {
		...
		given(this.noteService.findByEntryId(100L, readerId))
				.willReturn(Optional.of(new NoteDetails(noteId, 100L, "hello", new FrontMatter("Hello World!"), "https://example.com", new Author("demo1", OffsetDateTime.MIN), new Author("demo2", OffsetDateTime.MAX))));
		this.mockMvc.perform(get("/notes/100")
						.with(jwt().jwt(jwt -> jwt.subject(readerId.toString()).claim("scope", List.of("note:read")).build())))
				.andExpect(status().isOk())
	...
	void getNoteByNoteId_200() throws Exception {
		...
		given(this.noteService.findByNoteId(noteId, readerId))
				.willReturn(Optional.of(new NoteDetails(noteId, 100L, "hello", new FrontMatter("Hello World!"), "https://example.com", new Author("demo1", OffsetDateTime.MIN), new Author("demo2", OffsetDateTime.MAX))));
		this.mockMvc.perform(get("/notes/{noteId}", noteId)
						.with(jwt().jwt(jwt -> jwt.subject(readerId.toString()).claim("scope", List.of("note:read")).build())))
				.andExpect(status().isOk())
	...
	void deleteByEntryId() throws Exception {
		...
		this.mockMvc.perform(delete("/notes?entryId=100")
	...
	void subscribe_200() throws Exception {
		...
		this.mockMvc.perform(post("/notes/{noteId}/subscribe", noteId)
						.with(jwt().jwt(jwt -> jwt.subject(readerId.toString()).claim("scope", List.of("note:read")).build())))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.entryId").value(100L))
